...
async function verifyContract(contractName) {
  try {
    ...
    // Create the sourceFiles object with exact paths as in the import statements
    const exactPathSourceFiles = {};
    
    // Add the main contract file
    exactPathSourceFiles[`${contractName}.sol`] = sourceFiles[`${contractName}.sol`];
    
    // Add all other files with their exact paths
    for (const [filePath, content] of Object.entries(sourceFiles)) {
      if (filePath !== `${contractName}.sol`) {
        // Handle local imports
        exactPathSourceFiles[filePath] = content;
        
        // Handle @uniswap/v3-core imports
        if (filePath.includes('libraries/')) {
          exactPathSourceFiles[`@uniswap/v3-core/contracts/libraries/${path.basename(filePath)}`] = content;
        }
        
        if (filePath.includes('interfaces/')) {
          exactPathSourceFiles[`@uniswap/v3-core/contracts/interfaces/${path.basename(filePath)}`] = content;
        }
        
        if (filePath.includes('interfaces/callback/')) {
          exactPathSourceFiles[`@uniswap/v3-core/contracts/interfaces/callback/${path.basename(filePath)}`] = content;
        }
      }
    }
