            ...
            for (const log of receipt.logs) {
                try {
                    if (log.topics[0] === erc20TransferTopic && log.topics.length === 3) {
                        ...
                        // Handle the case where ids and values might be arrays or single values
                        let ids: any[] = [];
                        let values: any[] = [];
                        
                        // Check if ids exists and convert to array if needed
                        if (parsedLog.args.ids) {
                            ids = Array.isArray(parsedLog.args.ids) ? parsedLog.args.ids : [parsedLog.args.ids];
                        }
                        
                        // Check if values exists and convert to array if needed
                        if (parsedLog.args.values) {
                            values = Array.isArray(parsedLog.args.values) ? parsedLog.args.values : [parsedLog.args.values];
                        }
                        ...
                        if (ids.length > 0 && values.length > 0) {
                            ...
                            for (let i = 0; i < length; i++) {
                                // Skip if either id or value is not accessible
                                if (ids[i] === undefined || values[i] === undefined) {
                                    continue;
                                }
