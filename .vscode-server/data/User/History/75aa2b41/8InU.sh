...
log "Running migrations inside the Docker container..."
for migration_file in "$DIR/migrations"/*.sql; do
    filename=$(basename "$migration_file")
    log "Running migration: $filename"
    cat "$migration_file" | docker exec -i mainnet-indexer_postgres_1 psql -U new_user -d studio_indexer_new
    log "Migration completed: $filename"
done
