...
// Paths
const uniswapRoot = path.join(__dirname, '../..');
const v3PeripheryPath = path.join(uniswapRoot, 'node_modules/@uniswap/v3-periphery');
const v3CorePath = path.join(uniswapRoot, 'node_modules/@uniswap/v3-core');
...
/**
 * Get all source files for a contract
 * @param {string} contractName The name of the contract
 * @returns {object} The source files
 */
function getSourceFiles(contractName) {
  const sourceFiles = {};
  
  // Add the main contract file
  const mainContractPath = path.join(__dirname, '../contracts', `${contractName}.sol`);
  if (fs.existsSync(mainContractPath)) {
    sourceFiles[`${contractName}.sol`] = fs.readFileSync(mainContractPath, 'utf8');
  } else {
    console.error(`Main contract file not found: ${mainContractPath}`);
    process.exit(1);
  }
  
  // Add all files from v3-periphery
  if (fs.existsSync(v3PeripheryPath)) {
    const v3PeripheryFiles = glob.sync('**/*.sol', { cwd: v3PeripheryPath });
    for (const file of v3PeripheryFiles) {
      const filePath = path.join(v3PeripheryPath, file);
      sourceFiles[file] = fs.readFileSync(filePath, 'utf8');
    }
  } else {
    console.warn(`v3-periphery path not found: ${v3PeripheryPath}`);
  }
  
  // Add all files from v3-core
  if (fs.existsSync(v3CorePath)) {
    const v3CoreFiles = glob.sync('**/*.sol', { cwd: v3CorePath });
    for (const file of v3CoreFiles) {
      const filePath = path.join(v3CorePath, file);
      sourceFiles[file] = fs.readFileSync(filePath, 'utf8');
    }
  } else {
    console.warn(`v3-core path not found: ${v3CorePath}`);
  }
...
async function verifyContract(contractName) {
  try {
    ...
    const sourceFiles = getSourceFiles(contractName);
...
try {
  ...
  console.error('The "glob" package is required but not installed.');
