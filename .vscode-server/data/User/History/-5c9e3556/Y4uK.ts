...
export class StatsApiService extends ApiService {
    ...
    private async getValidatorsCount(req: Request, res: Response): Promise<void> {
        try {
            ...
            // Call the JSON-RPC method to get validators
            // This assumes the blockchain has a method like 'eth_validators' or similar
            // The actual method name may vary depending on the blockchain
            try {
                // Try different methods that might be available
                let validators;
                
                try {
                    // Try 'eth_validators' first
                    validators = await provider.send('eth_validators', []);
                } catch (e) {
                    try {
                        // Try 'system_validators' next
                        validators = await provider.send('system_validators', []);
                    } catch (e) {
                        // If both fail, use a fallback approach
                        validators = [];
                    }
                }
                ...
                // If the RPC method fails, use a fallback approach
                logger.warn('Failed to get validators from RPC, using fallback approach:', error);
                
                // Fallback: Use a fixed list of validators
                // In a real implementation, you would get this from a configuration file or database
                const validatorCount = 10; // Assume 10 validators
                ...
                res.json({
                    ...
                    latestBlock: latestBlockNumber,
                    blockTimestamp: latestBlock ? latestBlock.timestamp : null,
                    note: 'This is an estimate, not actual validator data'
                });
            }
        } catch (error: any) {
