    ...
    private async getValidatorsPayout(req: Request, res: Response): Promise<void> {
        try {
            ...
            for (const validatorAddress of validatorAddresses) {
                ...
                // Calculate the validator's share of the total block rewards
                // Since we can't determine which validator mined which block (most blocks have miner=0x0),
                // we'll assume an equal distribution of block rewards among validators
                const blockReward = ethers.utils.parseEther('0.1'); // Assume 0.1 STO per block
                const validatorShare = latestBlockNumber / validatorAddresses.length;
                const validatorPayout = blockReward.mul(Math.floor(validatorShare));
                ...
                validatorPayouts.push({
                    ...
                    estimatedBlocksMined: Math.floor(validatorShare),
        ...
        try {
            ...
            for (const validatorAddress of validatorAddresses) {
                try {
                    ...
                    const validatorIpAddresses = validatorRpcUrls.map(url => rpcIpAddresses[url]).filter(Boolean);
