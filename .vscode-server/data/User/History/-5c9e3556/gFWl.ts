    ...
    private async getValidatorsPayout(req: Request, res: Response): Promise<void> {
        try {
            ...
            for (const validatorAddress of validatorAddresses) {
                ...
                // Get blocks mined by this validator
                // Since we don't have a direct method to query blocks by miner,
                // we'll need to loop through blocks and check the miner
                const blocksMinedByValidator = [];
                
                // Get a sample of recent blocks to check for this validator
                // We'll look at the last 10000 blocks for better coverage
                const startBlock = Math.max(0, latestBlockNumber - 10000);
                
                for (let i = startBlock; i <= latestBlockNumber; i++) {
                    const block = await this.database.getBlock(i);
                    if (block && block.miner && block.miner.toLowerCase() === validatorAddress.toLowerCase()) {
                        blocksMinedByValidator.push(block);
                        if (blocksMinedByValidator.length >= 100) {
                            // Limit to 100 blocks for performance
                            break;
                        }
                    }
                }
        ...
        try {
            ...
            for (const validatorAddress of validatorAddresses) {
                try {
                    ...
                    // Get blocks mined by this validator
                    const startBlock = Math.max(0, latestBlockNumber - 10000); // Look at the last 10000 blocks
                    let blocksMined = 0;
