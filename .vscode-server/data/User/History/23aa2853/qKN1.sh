...
DB_NAME=${DB_NAME:-analytics_data_new}
...
check_postgres() {
  ...
  if ! docker exec mainnet-indexer_postgres_1 pg_isready -U $DB_USER > /dev/null 2>&1; then
...
check_database() {
  ...
  if ! docker exec mainnet-indexer_postgres_1 psql -U $DB_USER -d studio_indexer_new -c '\l' > /dev/null 2>&1; then
    log "ERROR: Cannot connect to PostgreSQL server"
    return 1
  fi

  # Check if the database exists
  if ! docker exec mainnet-indexer_postgres_1 psql -U $DB_USER -d studio_indexer_new -c "SELECT datname FROM pg_database WHERE datname = '$DB_NAME';" | grep -q $DB_NAME; then
    log "ERROR: Database '$DB_NAME' does not exist"
    return 1
  fi

  # Check if the blocks table exists
  if ! docker exec mainnet-indexer_postgres_1 psql -U $DB_USER -d $DB_NAME -c "SELECT to_regclass('public.blocks');" | grep -q blocks; then
...
check_honeypot() {
  ...
  if ! docker exec mainnet-indexer_postgres_1 psql -U $DB_USER -d studio_indexer_new -c "SELECT datname FROM pg_database WHERE datname = '$HONEYPOT_DB';" | grep -q $HONEYPOT_DB; then
...
restore_honeypot() {
  ...
  if ! docker exec mainnet-indexer_postgres_1 psql -U $DB_USER -d studio_indexer_new -c "CREATE DATABASE $HONEYPOT_DB;" > /dev/null 2>&1; then
    log "ERROR: Failed to create honeypot database"
    return 1
  fi

  # Create dummy table and insert data
  if ! docker exec mainnet-indexer_postgres_1 psql -U $DB_USER -d $HONEYPOT_DB -c "
    CREATE TABLE blockchain_data (id SERIAL PRIMARY KEY, block_hash VARCHAR(66), transaction_hash VARCHAR(66), value NUMERIC(78), timestamp TIMESTAMP);
    INSERT INTO blockchain_data (block_hash, transaction_hash, value, timestamp) VALUES ('0x4509784dcc864fa2331ed5cd22ec5ba11cf6b2d129db968fd8676466ae4f7c7d', '0x8005de76ad1487145dcfada2f3d0aca4940123f69d550c9f4c978c8a04af73b3', 1000000000000000000, NOW());
  " > /dev/null 2>&1; then
...
create_database() {
  ...
  # Create the database
  if ! docker exec mainnet-indexer_postgres_1 psql -U $DB_USER -d studio_indexer_new -c "CREATE DATABASE $DB_NAME;" > /dev/null 2>&1; then
  ...
  for migration in /project/migrations/*.sql; do
    ...
    if ! docker exec mainnet-indexer_postgres_1 psql -U $DB_USER -d $DB_NAME -f /tmp/$(basename $migration) > /dev/null 2>&1; then
...
backup_database() {
  ...
  # Create the backup
  if ! docker exec mainnet-indexer_postgres_1 pg_dump -U $DB_USER -d $DB_NAME > $backup_file 2>/dev/null; then
...
restore_from_backup() {
  ...
  if ! cat /tmp/restore.sql | docker exec -i mainnet-indexer_postgres_1 psql -U $DB_USER -d $DB_NAME > /dev/null 2>&1; then
...
mkdir -p \$BACKUP_DIR
docker exec mainnet-indexer_postgres_1 pg_dump -U $DB_USER -d \$DB_NAME > \$backup_file 2>/dev/null
gzip \$backup_file
find \$BACKUP_DIR -name "\${DB_NAME}_*.sql.gz" -type f -mtime +7 -delete
EOF
