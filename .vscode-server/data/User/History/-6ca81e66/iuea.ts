...
export class NFTsApiService extends ApiService {
    ...
    private async getNFTMetadata(req: Request, res: Response): Promise<void> {
        try {
            ...
            // Get the NFT token from the database
            const nfts = await this.database.getNFTsByOwner(
                null, // We don't know the owner, so we'll search by token address and token ID
                {
                    tokenAddress,
                    limit: 1,
                    offset: 0
            ...
            // Find the NFT with the matching token ID
            const nft = nfts.find(n => n.tokenId === tokenId);
            if (!nft) {
                // If we can't find the NFT, just return the metadata
                res.json({ tokenAddress, tokenId, metadata });
                return;
            }
            
            // Return the NFT with the metadata
            res.json({
                ...nft,
                metadata
            });
