...
const CONTRACT_CREATION_TOPIC = '0x8be0079c531659141344cd1fd0a4f28419497f9722a3daafe3b4186f6b6457e0'; // OwnershipTransferred(address,address)
...
class Blockchain {
    ...
    async getContractOwner(contractAddress: string): Promise<string | null> {
        try {
            ...
            return await this.callContractMethod(contractAddress, OWNABLE_ABI, 'owner', []);
        } catch (error) {
    ...
    async detectNewContracts(fromBlock: number, toBlock: number): Promise<ContractData[]> {
        try {
            ...
            for (let blockNumber = fromBlock; blockNumber <= toBlock; blockNumber++) {
                try {
                    const { block, transactions } = await this.getBlockWithTransactions(blockNumber);
                            ...
                            try {
                                ...
                                if (receipt && receipt.contractAddress) {
                                    ...
                                    const contractData: ContractData = {
                                        ...
                                        balance: (await this.getBalance(receipt.contractAddress)).toString()
                                    };
