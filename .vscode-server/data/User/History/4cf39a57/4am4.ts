...
export class TokensApiService extends ApiService {
    ...
    private async getAccountBalances(req: Request, res: Response): Promise<void> {
        try {
            ...
            // Get native STO balance from the database
            const accountQuery = await this.database.pool.query(
            ...
            if (accountQuery.rows.length > 0) {
                nativeBalance = accountQuery.rows[0].balance;
            } else {
                // If not in database, get from blockchain
                const blockchainBalance = await blockchain.getBalance(address);
                nativeBalance = blockchainBalance.toString();
            }
    ...
    private async getAddressTokens(address: string): Promise<any[]> {
        try {
            ...
            const tokens = await Promise.all(tokenBalances.map(async (tokenBalance) => {
                try {
                    // Get token information
                    const name = await blockchain.getTokenName(tokenBalance.tokenAddress) || '';
                    const symbol = await blockchain.getTokenSymbol(tokenBalance.tokenAddress) || '';
                    let decimals = 18;
