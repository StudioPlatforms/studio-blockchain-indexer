...
export class TokensApiService extends ApiService {
    ...
    private async getAddressTokens(address: string): Promise<any[]> {
        try {
            ...
            // If no balances found in database, try blockchain fallback
            if (tokenBalances.length === 0) {
                logger.info(`No token balances found in database for ${address}, trying blockchain fallback`);
                ...
                if (blockchainBalances.length > 0) {
                    ...
                    // Store these balances in the database for future use
                    for (const balance of blockchainBalances) {
                        try {
                            ...
                            logger.info(`Stored token balance for ${balance.tokenAddress} in database`);
                        } catch (error) {
                            logger.error(`Error storing token balance for ${balance.tokenAddress}:`, error);
                        }
                    }
                }
            }
